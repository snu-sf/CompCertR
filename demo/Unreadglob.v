...
Definition filter_instr (kept: ident -> bool) (i: instruction): instruction :=
  match i with
  | Istore _ addr _ _ s =>
    if List.forallb (fun id => kept id) (globals_addressing addr)
    then i
    else Inop s
  | _ => i
  end
...
Definition map_globdef (kept: ident -> bool) (gd: globdef fundef unit): globdef fundef unit :=
  match gd with
  | Gfun (Internal f) =>
    Gfun (Internal (transf_function (fun _ i => filter_instr kept i) f))
  | _ => gd
  end
.

Definition map_globdefs (kept: ident -> bool) (gds: list (ident * globdef fundef unit)) :=
  map (fun igd => (igd.(fst), map_globdef kept igd.(snd))) gds
.
...
Definition check_instr (defs: list ident) (i: instruction): bool :=
  match i with
  | Istore _ addr _ _ _ => List.forallb (fun id => in_dec (peq) id defs) (globals_addressing addr)
  | _ => true
  end
.

Definition PTree_forallb {X} (f: ident -> X -> bool) (px: PTree.t X): bool :=
  PTree.fold (fun s k x => f k x && s) px true
.

Definition check_globdef (defs: list ident) (gd: globdef fundef unit): bool :=
  match gd with
  | Gfun (Internal f) =>
    PTree_forallb (fun _ i => check_instr defs i) f.(fn_code)
  | _ => true
  end
.

Definition check_program (p: program): bool :=
  List.forallb (fun igd => check_globdef (p.(prog_main) :: p.(prog_defs_names)) igd.(snd)) p.(prog_defs)
.
...
Definition transform_program (p: program) : res program :=
  ...
  | Some used =>
      if IS.for_all (global_defined p pm) used then
        OK {| prog_defs := filter_globdefs used nil (map_globdefs (fun id => IS.mem id used) (List.rev p.(prog_defs)));
              prog_public := p.(prog_public);
              prog_main := p.(prog_main) |}
      else
        Error (msg "Unusedglob: reference to undefined global")
  end
  else Error (msg "UnusedglobStrong: ill formed program")
  .
