  ...
  | wt_Ebuiltin: forall ef tyargs rargs ty,
      ...
      \/ (tyargs = Tcons type_bool (Tcons ty (Tcons ty Tnil))
          /\ let t := typ_of_type ty in
             let sg := mksignature (AST.Tint :: t :: t :: nil) (Some t) cc_default true in
...
Definition retype_fundef (ce: composite_env) (e: typenv) (fd: fundef) : res fundef :=
  ...
  | External ef args res cc => assertion(signature_eq (signature_of_type args res cc) (ef_sig ef)); OK fd
  end.
...
+ simpl in B. set (T := typ_of_type ty) in *. 
  set (sg := mksignature (AST.Tint :: T :: T :: nil) (Some T) cc_default true) in *.
